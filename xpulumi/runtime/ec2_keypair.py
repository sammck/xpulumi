...
class Ec2KeyPair:
  ...
  def __init__(
    ...
    if use_config and public_key is None and public_key_file is None:
      ...
      pulumi.info(f"{cfg_prefix}CFG ssh_public_key={public_key} public_key_file={public_key_file}")
    ...
    if not self._committed:
      ...
      if keypair_id is None:
        if public_key is None:
          self.set_public_key(filename="~/.ssh/id_rsa.pub")
          public_key = self.public_key
          assert not public_key is None
