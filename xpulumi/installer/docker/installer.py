...
class BinFmtEntry:
  ...
  def get_package_name(self) -> str:
    ...
    if result == '':
      raise XPulumiError(f"Unable to read binfmts package name from {self._pathname}")
    return result
  ...
  def is_fixed_binary(self) -> bool:
    ...
    if fline is None or not fline in ("", "yes"):
      raise XPulumiError(f"Unable to read binfmts fixed binary status from {self._pathname}")
...
def install_docker(force: bool=False):
  ...
  if need_client_install:
    ...
    if not docker_is_installed():
      raise XPulumiError("Docker client still not found in PATH after install/upgrade.")
