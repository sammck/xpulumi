...
def install_aws_cli(force: bool=False):
  ...
  if need_client_install:
    ...
    if not aws_cli_is_installed():
      raise XPulumiError("AWS CLI still not found in PATH after install/upgrade.")
    ...
    if prog != os.path.join(home_local, 'bin', 'aws'):
      raise XPulumiError(f"The AWS CLI in PATH ({prog}) is not the most recently installed.")
